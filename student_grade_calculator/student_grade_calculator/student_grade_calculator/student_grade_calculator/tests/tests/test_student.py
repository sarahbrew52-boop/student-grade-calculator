...
def test_student_transcript_shape():
    s = Student("S001", "Ama Mensah")
    s.add_score("Math101", "Final", 88)
    t = s.transcript()
    assert t["student_id"] == "S001"
    assert "Math101" in t["courses"]
    assert t["courses"]["Math101"]["letter_grade"] == "B"
